...
const ProductCard = ({ product }) => {
  ...
  const { addCart: addItem, isAdding } = useCart();
  ...
  return (
    <div className="group relative p-3 bg-white rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300 border border-slate-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-5">
        ...
        <button
          disabled={isAdding}
          className="text-xs md:text-sm flex items-center justify-center py-2 px-4 gap-2 text-white bg-(--main-color) rounded-xl transition-colors duration-300 cursor-pointer disabled:opacity-60"
