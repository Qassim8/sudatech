...
const ProductDetail = () => {
  const { id } = useParams();
  const { singleProduct: product, singleProductLoading } = useProducts({
    productId: id,
  });
        ...
        {singleProductLoading ? (
          ...
          <div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
              ...
              <div>
                ...
                <p className="mt-6 text-(--text-color)">
                  {product?.description}
                </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-5">
            ...
            {!productsLoad &&
              products?.items
                ?.filter((p) => {
                  // 1. استخراج الـ ID للقسم الخاص بالمنتج المعروض حالياً (يدعم المسميين)
                  const currentCatId =
                    product?.category?.documentId ||
                    product?.categories?.documentId;

                  // 2. استخراج الـ ID للقسم الخاص بالمنتج الذي نمر عليه في المصفوفة
                  const itemCatId =
                    p?.category?.documentId || p?.categories?.documentId;

                  // إذا كانت البيانات لا تزال تُجلب (currentCatId غير موجود)، لا تعرض شيئاً مؤقتاً لحين اكتمال التحميل
                  if (!currentCatId || !itemCatId) return false;

                  // 3. الشرط: يجب أن يتطابق القسم، وألا يكون المنتج هو نفسه المنتج المعروض حالياً
                  return (
                    itemCatId === currentCatId &&
                    p?.documentId !== product?.documentId
                  );
                })
                .map((p) => <ProductCard key={p.documentId} product={p} />)}
          </div>
        </section> */}
          </div>
        )}
